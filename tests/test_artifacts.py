...
def test_committed_fixtures_were_built_with_the_targeted_dbt_version():
    """The fixtures must come from the dbt version the client actually runs.

    Regenerating them with dbt Fusion would silently stop proving that the
    shipped CI workflow works, because Fusion has no `dbt docs generate`.
    """
    for name in ("manifest.json", "catalog.json"):
        version = json.loads((FIXTURES / name).read_text())["metadata"]["dbt_version"]
        assert version.startswith("1."), (
            f"fixtures/{name} was generated by dbt {version}. Regenerate it with "
            "the pinned dbt Core in .venv: scripts/rebuild_fixture.sh"
        )
